...
class MosViz:
    def __init__(self, env, res=30, fps=30, controllable=False):
        ...
        colors = {}
        for i, obj in enumerate(env.get_world_state()):
            colors[obj.objid] = object_color(obj.objid, i)
        self._target_colors = colors

    def _make_gridworld_image(self, r=30): # create grid world based on Environment Object
        # Preparing 2d array
        ...
        for obj in state:
            pose = obj.pose
            if obj.objtype == "robot":
                arr2d[pose[0], pose[1]] = 0  # free grid
            elif obj.objtype == "obstacle":
                arr2d[pose[0], pose[1]] = 1  # obstacle
            elif obj.objtype == "box":
                arr2d[pose[0], pose[1]] = 2  # box
            elif obj.objtype == "target":
                arr2d[pose[0], pose[1]] = 3  # target
        ...
        for x in range(w):
            for y in range(l):
                if arr2d[x, y] == 0:  # free
                    ...
                    cv2.rectangle(
                        img, (y * r, x * r), (y * r + r, x * r + r), (165, 165, 100), -1
                    )
        ...
        # cv2.imwrite(f'/home/miskojones/Projects/decision_making/project/imgs/init_template.jpg', img)
        # exit()
        return img
    ...
    # use this update render during example
    def update(self, robot_id, action, observation={}, viz_observation={}, belief={}): # action, observation, viz_observation not used during exmaple. State updated before rerender
        """
    ...
    def draw_robot(img, x, y, th, size, color=(255, 12, 12)):
        ...
        cv2.line(img, (y + radius, x + radius), endpoint, color, 2) # line showing robot orientation

    # @staticmethod
    # def draw_observation(img, z, rx, ry, rth, r, size, color=(12, 12, 255)): # observations obtained from find and look actions
    ...
    def on_event(self, event): # respond keyboard event
        # TODO: Keyboard control multiple robots
        robot = self._env.get_robot_state()
        robot_id = robot.objid  # Just pick the first one.
    ...
    def on_render(self):
        ...
        robot = self._env.get_robot_state()  # Just pick the first one.
        robot_id = robot.objid
        rx, ry = robot.pose
        fps_text = "FPS: {0:.2f}".format(self._clock.get_fps())
        last_action = self._last_action.get(robot_id, None)
        orientation = 0
        if last_action == MoveWest2D:
            orientation = np.pi
        elif last_action == MoveSouth2D:
                orientation = np.pi/2
        elif last_action == MoveNorth2D:
            orientation = np.pi * 3/2
    ...
    def render_env(self, display_surf):
        ...
        robot_states = [self._env.get_robot_state()]
        for i, robot_state in enumerate(robot_states):
            rx, ry = robot_state.pose
            ...
            #     )
            orientation = 0
            last_action = self._last_action[robot_state.objid]
            if last_action == MoveWest2D:
                orientation = np.pi
            elif last_action == MoveSouth2D:
                orientation = np.pi/2
            ...
            MosViz.draw_robot(
                img, rx * r, ry * r, orientation, r, color=(12, 255 * (0.8 * (i + 1)), 12)
            )
        pygame.surfarray.blit_array(display_surf, img)
        # cv2.imwrite(f'/home/miskojones/Projects/decision_making/project/imgs/img_{}.jpg', img)
